describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      ...
      console.log(`Item ${i+1}: title="${data.title}", price="${data.price}", image = "${data.image}"`);
    }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
    const productItem = (await page.$$('product-item'))[0];

    const shadowRoot = await productItem.getProperty('shadowRoot');
    const button = await shadowRoot.$('button');

    await button.click();
    const innerTestHandle = await button.getProperty('innerText');
    const innerText = await innerTestHandle.jsonValue();
    
    expect(innerText).toBe('Remove from Cart');
    await button.click(); // Click again to reset state
  }, 2500);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.$$eval('product-item', items => {
      items.forEach(item => {
        const btnText = item.shadowRoot.querySelector('button').innerText;
        if (btnText !== 'Remove from Cart') throw new Error('Button did not persist');
      });
    });
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.$$eval('product-item', items => {
      items.forEach(item => {
        const btnText = item.shadowRoot.querySelector('button').innerText;
        if (btnText !== 'Add to Cart') throw new Error('Button did not reset');
      });
    });
